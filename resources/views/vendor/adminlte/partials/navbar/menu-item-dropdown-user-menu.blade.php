@php
  ...
  $profile_url = [
    'loc' => View::getSection('profile_loc') ?? config('adminlte.profile_url.loc', 'sub'),
    'path' => View::getSection('profile_url') ?? config('adminlte.profile_url.path', 'profile')
  ];
@endphp
      ...
      <li class="user-header {{ config('adminlte.usermenu_header_class', 'bg-primary') }}
        ...
        <p class="@if(!config('adminlte.usermenu_image')) mt-0 @endif">
          ...
          @if(config('adminlte.usermenu_desc'))
            <small>{{ Auth::user()->adminlteDesc() }}</small>
          @endif
        </p>
      </li>
    @else
